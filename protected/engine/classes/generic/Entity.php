...
abstract class Entity extends Component 
{
    ...
    const MEDIA_RESOURCES_KEY = '_media';
    ...
    public function appendProp($sKey, $sSubKey, $xVal = null)
    {
        ...
        if (null !== $xData && !is_array($xData)) {
            $this->aProps[$sKey] = array($xData);
        } elseif (null === $xData) {
            $this->aProps[$sKey] = [];
        }
        if (null === $sSubKey) {
    ...
    {
        if (isset($this->aProps[$sKey]) || array_key_exists($sKey, $this->aProps)) {
     ...
     * @return string
     */
    public function getPropStr($sKey, $sDefault = null)
    {
        return (string)$this->getProp($sKey, $sDefault);
    }
     ...
     * @return bool
     */
    public function getPropBool($sKey, $bDefault = null)
    {
        return (bool)$this->getProp($sKey, $bDefault);
    }
     ...
     * @return string
     */
    public function getPropInt($sKey, $iDefault = null)
    {
        return (string)$this->getProp($sKey, $iDefault);
    }

    /**
     * Gets languages associated property of entity
     *
     * @param string $sKey
     * @param mixed  $xDefault
     * @param string $sLang`
    ...
    public function getLangSuffixProp($sKey, $sLang = null, $xDefault = null)
    {
        ...
        if (null === $sResult) {
            $sResult = $this->getProp($sKey . '_' . \E::Module('Lang')->getDefaultLang());
            if (null === $sResult && null !== ($sVal = $this->getProp($sKey . '_en'))) {
                return $sVal;
            }
            if (null === $sResult && null !== ($sVal = $this->getProp($sKey . '_ru'))) {
                return $sVal;
            }
            if (null === $sResult && null !== ($sVal = $this->getProp($sKey))) {
                return $sVal;
            }
            return $xDefault;
        }
    ...
    public function getLangTextProp($sKey, $sLang = null)
    {
        ...
        if (!$sResult) {
            ...
            if (0 === strpos($sResult, '{{') && substr($sResult, -2) === '}}') {
                if (null !== ($sText = \E::Module('Lang')->get('[' . $sLang . ']' . substr($sResult, 2, strlen($sResult) - 4)))) {
                    $sResult = $sText;
                }
            }
    ...
    protected function _getExpandedData($sSubKey = null)
    {
        ...
        if ($sSubKey && in_array($sSubKey, [self::EXP_KEY_SET, self::EXP_KEY_DEL, self::EXP_KEY_MOD], true)) {
    ...
    protected function _propExpandedInclude($sExpIdx, $sKey, $xVal)
    {
        ...
        if (in_array($sExpIdx, [self::EXP_KEY_SET, self::EXP_KEY_DEL, self::EXP_KEY_MOD], true)) {
    ...
    {
        return isset($this->aProps[$sKey]) || array_key_exists($sKey, $this->aProps);
    }
    ...
    {
        if ((null === $aKeys) || (is_array($aKeys) && !count($aKeys))) {
            return $this->aProps;
        }

        if (is_bool($aKeys)) {
            $aKeys = array_keys($this->aProps);
        ...
        foreach ((array)$aKeys as $sKey) {
            if (!$bRecursively) {
                ...
                if (is_object($xValue) && $xValue instanceOf self) {
    ...
    {
        return array_values($this->aProps);
    }
    ...
    {
        $aInfo = \E::getInstance()->getClassInfo($this, \E::CI_ENTITY);
        if (isset($aInfo[\E::CI_ENTITY])) {
            return $aInfo[\E::CI_ENTITY];
        }
    ...
    public function __call($sName, $aArgs)
    {
        ...
        throw new \RuntimeException(sprintf('Method "%s" not defined!', $sName));
    }
    ...
    public function _createValidators()
    {
        ...
        foreach ($this->aValidateRules as $aRule) {
            if (isset($aRule[0], $aRule[1])) {
                $aValidators[] = \E::Module('Validate')->CreateValidator($aRule[1], $this, $aRule[0], array_slice($aRule, 2));
            } else {
    ...
    {
        if ($sField === null) {
            $aAllErrors = $this->_getValidateErrors();
            $aErros = reset($aAllErrors);
            return reset($aErros);
        }
    ...
    public function checkCustomRules($aRules, $bConcatenateResult = FALSE, $sOwnerClassName = FALSE)
    {
        ...
        if (!(FALSE === ($data = \E::Module('Cache')->getTmp($sCacheKey)))) {
            return $data;
        }
        ...
        if (!$sOwnerClassName) {
            $aOwnerClassName = \E::getInstance()->getClassInfo($this, \E::CI_MODULE);
        ...
        foreach ($aRules as $sMethodName => $xRule) {
            ...
            if ($sTmpMethodName && $aTmpMethodParams !== FALSE) {
                if (strpos($sTmpMethodName, ':')) {
                    ...
                    //$xCallResult = call_user_func_array([$this, $sTmpMethodName], $aTmpMethodParams);
                    $oModule = \E::Module($sOwnerClassName);
                    $sTmpMethodName = \F::strCamelize($sTmpMethodName);
                    $xCallResult = call_user_func_array([$oModule, $sTmpMethodName], $aTmpMethodParams);
                }
                if ($bConcatenateResult) {
        ...
        \E::Module('Cache')->setTmp($bResult, $sCacheKey);
    ...
    public function delMediaTypes($sMediaType)
    {
        if (is_array($this->aMResourceTypes)) {
            $iKey = array_search($sMediaType, $this->aMResourceTypes, true);
            if ($iKey !== false) {
    ...
    /**
     * @param string                               $sType
     * @param ModuleMedia_EntityMediaRel[] $data
     */
    public function setMediaResources($sType, $data)
    {
        ...
        $aMedia = $this->getProp(static::MEDIA_RESOURCES_KEY);
        if (is_null($aMedia)) {
            $aMedia = [];
        }
        $aMedia[$sType] = $data;
        $this->setProp(static::MEDIA_RESOURCES_KEY, $aMedia);
    }
    ...
    public function getMediaResources($sType = null)
    {
        ...
        if ($iEntityId) {
            $aMedia = $this->getProp(static::MEDIA_RESOURCES_KEY);
            if (null === $aMedia && ($sType || $this->getMediaTypes())) {
                ...
                $aImages = \E::Module('Uploader')->GetTargetImages($aTargetTypes, $iEntityId);
                ...
                $aImages = \E::Module('Uploader')->GetTargetImages($sType, $iEntityId);
                if (!empty($aImages)) {
