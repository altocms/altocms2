...
class ModuleValidate_EntityValidatorNumber extends ModuleValidate_EntityValidator {
    ...
    public function validate($sValue) {
        ...
        if (is_array($sValue)) {
            return $this->getMessage(\E::Module('Lang')->get('validate_number_must_number', null, false), 'msg');
        }
        ...
        if ($this->integerOnly) {
            if (!preg_match($this->integerPattern, "$sValue")) {
                return $this->getMessage(\E::Module('Lang')->get('validate_number_must_integer', null, false), 'msg');
            }
        } else {
        ...
        if ($this->min !== null && $sValue < $this->min) {
            return $this->getMessage(
                \E::Module('Lang')->get('validate_number_too_small', null, false), 'msgTooSmall', array('min' => $this->min)
            );
        }
        if ($this->max !== null && $sValue > $this->max) {
            return $this->getMessage(
                \E::Module('Lang')->get('validate_number_too_big', null, false), 'msgTooBig', array('max' => $this->max)
            );
        }
        return true;
    }
