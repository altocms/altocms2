...
class ModuleImg extends Module {
    ...
    protected $aAvailableDrivers = [];
    protected $aInitDrivers = [];
    ...
    public function getOptions($sFileExtension = '*', $sPreset = null, $aOptions = array()) {
        ...
        $aConfigOptions = \E::Module('Uploader')->GetConfig($sFileExtension, $sPreset);
     ...
     * @return bool
     */
    public function transformFile($sFile, $xPreset = 'default', $aOptions = [])
    {
        if (is_array($xPreset)) {
            $aOptions = $xPreset;
            $xPreset = null;
        ...
        if ($xPreset) {
            $aPresetOptions = $this->getOptions($sFile, $xPreset, $aOptions);
        } else {
            $aPresetOptions = [];
        }
    ...
    public function originalFile($sFile, &$aOptions)
    {
        ...
        if (preg_match('~^(.+)-(\d*x\d*)(\-([a-z]+))?\.[a-z]+$~i', $sFile, $aMatches)) {
            ...
            $aOptions['width'] = ($nW ? (int)$nW : null);
            $aOptions['height'] = ($nH ? (int)$nH : null);
    ...
    public function buildHTML($sUrl, $aParams)
    {
        ...
        if (isset($aParams['align']) && in_array($aParams['align'], ['left', 'right', 'center'], true)) {
    ...
    protected function _getDefaultSkinImage($sFile, $sPrefix, $iSize)
    {
        ...
        if (preg_match('/^' . preg_quote($sPrefix, '/') . '_([a-z0-9-]+)(_(male|female|[a-z]+))?(\.([\-0-9a-z]+))?(\.([a-z]+))$/i', $sName, $aMatches)) {
            ...
            if ($sSkin) {
                ...
                $sPath = \C::get('path.skins.dir') . $sSkin . '/assets/images/avatars/';
                if (!is_dir($sPath)) {
                    // старая структура скина
                    $sPath = \C::get('path.skins.dir') . $sSkin . '/images/';
                }
        ...
        if ($aFiles = glob($sPath . $sName . '.*')) {
            ...
            foreach ($aFiles as $sFile) {
                if (preg_match('/_(\d+)x(\d+)\./', basename($sFile), $aMatches)) {
                    $nI = (int)max($aMatches[1], $aMatches[2]);
