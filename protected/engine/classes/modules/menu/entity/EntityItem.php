...
class ModuleMenu_EntityItem extends Entity {
    ...
    public function init() {
        ...
        if (!isset($this->aProps['item_flags'])) {
            $this->aProps['item_flags'] = $this->formHtml();
        }
    }
    ...
    public function getUrl() {
        ...
        $sLink = isset($this->aProps['item_url']) ? \C::get($this->aProps['item_url']) : NULL;
        if (!$sLink) {
            return isset($this->aProps['item_url']) ? $this->aProps['item_url'] : NULL;
        }
    ...
    public function isEnabled($sType = FALSE) {
        ...
        // Проверим по скину
        if ($this->getOptions() && $this->getOptions()->getSkin() && $this->getOptions()->getSkin() != \E::Module('Viewer')->GetConfigSkin()) {
            return FALSE;
        } else {
            // Если шкурка совпала, то проверим по теме
            if ($this->getOptions() && $this->getOptions()->getTheme() && $this->getOptions()->getTheme() != \E::Module('Viewer')->GetConfigTheme()) {
                return FALSE;
            }
        }
    ...
    public function getLangText($sTextTemplate, $aReplace = array(), $sLang = NULL) {
        ...
        return preg_replace_callback('~\{\{\S*\}\}~U', function ($aMatches) use ($aReplace, $sLang) {
            return \E::Module('Lang')->text($aMatches[0], $aReplace, true, $sLang);
        }, $sTextTemplate);
    }
    ...
    public function getTitle() {
        ...
        $aActiveRule = isset($this->aProps['item_title']) ? $this->aProps['item_title'] : '';
    ...
    public function getDescription(){
        ...
        $this->_description = isset($this->aProps['item_description']) ? $this->aProps['item_description'] : NULL;
    ...
    public function getText() {
        ...
        /** @var callable[]|[][] $aActiveRule Правило вычисления активности */
        $aActiveRule = isset($this->aProps['item_text']) ? $this->aProps['item_text'] : '';
    ...
    public function getOptions() {
        ...
        $aOptions = isset($this->aProps['item_options']) ? $this->aProps['item_options'] : NULL;
        if ($this->getSubMenuId() && preg_match('~submenu_[a-f0-9]{10}~', $this->getSubMenuId())) {
            $aOptions = \E::getEntity('Menu_ItemOptions', \C::get('menu.submenu.options'));
        }
        return $aOptions;
    }
    ...
    public function getOn() {
        ...
        return isset($this->aProps['item_on']) ? $this->aProps['item_on'] : NULL;
    }
    ...
    public function getOff() {
        ...
        return isset($this->aProps['item_off']) ? $this->aProps['item_off'] : NULL;
    }
    ...
    public function getType() {
        ...
        $xData = isset($this->aProps['item_type']) ? $this->aProps['item_type'] : NULL;
    ...
    public function getDisplay() {
        ...
        /** @var callable[]|[][] $aActiveRule Правило вычисления активности */
        $aActiveRule = isset($this->aProps['item_display']) ? $this->aProps['item_display'] : TRUE;
    ...
    public function getShow() {
        ...
        /** @var callable[]|[][] $aActiveRule Правило вычисления активности */
        $aActiveRule = isset($this->aProps['item_show']) ? $this->aProps['item_show'] : TRUE;
    ...
    public function getId() {
        ...
        return isset($this->aProps['item_id']) ? $this->aProps['item_id'] : NULL;
    }
    ...
    public function getSubMenuId() {
        ...
        return isset($this->aProps['item_submenu']) ? $this->aProps['item_submenu'] : NULL;
    }
    ...
    public function getActive() {
        ...
        /** @var callable[]|[][] $aActiveRule Правило вычисления активности */
        $aActiveRule = isset($this->aProps['item_active']) ? $this->aProps['item_active'] : FALSE;
    ...
    public function getHtml() 
    {
        ...
        if ($aParams['[[link_text]]'] = $this->getText()) {
            ...
            if (($this->getOptions() && ($sImageUrl = $this->getOptions()->getImageUrl()))) {
                ...
                $aParams['[[link_image]]'] = "<img {$this->aProps['item_flags']['item_image_class']} {$sImageAlt} src='$sImageUrl' />";
            }
            $aParams['[[link_title]]'] = '';
            ...
            if ($sActive && $this->aProps['item_flags']['link_class']) {
                $sLinkClass = str_replace('[[link_active]]', $this->aProps['item_flags']['link_active'], $this->aProps['item_flags']['link_class']);
            } else if ($sActive && !$this->aProps['item_flags']['link_class']) {
                $sLinkClass = "class='{$this->aProps['item_flags']['link_active']}'";
            } else if (!$sActive && $this->aProps['item_flags']['link_class']) {
                $sLinkClass = str_replace('[[link_active]]', '', $this->aProps['item_flags']['link_class']);
            } else {
                $sLinkClass = '';
            }
            $aParams['[[link_class]]'] = $sLinkClass;

            $sCurrentLink = "<a {$this->aProps['item_flags']['link_id']} [[link_class]] [[link_title]] {$this->aProps['item_flags']['link_data']} href='{$this->aProps['item_flags']['item_url']}'>{$this->aProps['item_flags']['item_icon']}[[link_image]][[link_text]]</a>";
        } else {
            $sCurrentLink = '';
        }
        ...
        if ($sActive && $this->aProps['item_flags']['item_class']) {
            $sItemClass = str_replace('[[link_active]]', $this->aProps['item_flags']['item_active'], $this->aProps['item_flags']['item_class']);
        } else if ($sActive && !$this->aProps['item_flags']['item_class']) {
            $sItemClass = "class='{$this->aProps['item_flags']['item_active']}'";
        } else if (!$sActive && $this->aProps['item_flags']['item_class']) {
            $sItemClass = str_replace('[[link_active]]', '', $this->aProps['item_flags']['item_class']);
        } else {
            $sItemClass = '';
        }
        ...
        $sHtml = "<li [[item_show]] [[item_class]] [[item_title]] {$this->aProps['item_flags']['item_data']}>{$sCurrentLink}{$this->aProps['item_flags']['item_submenu']}</li>";
