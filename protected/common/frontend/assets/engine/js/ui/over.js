...
(function($) {
    ...
    /**
     * Constructs over objects
     * @constructor
     * @class Over
     * @param {Object} toggle  Toggle element
     * @param {Object} options Options
     */
    var Over = ls.over = function (toggle, options) {
        this.init('over', toggle, options);
    };

    /**
     * Hide all overs
     */
    Over.hideAll = function (type) {
     ...
     * Init plugin
     */
    Over.initPlugin = function (type, elements, options, variable, value) {
        ...
        if (options.selector) {
            $(elements).on(options.event, options.selector, function (e) {
                ! $(this).data('object') && Over.initPluginElements(type, $(this), options, variable, value);
                $(this).data('object').toggle(e);
            });
        } else {
            Over.initPluginElements(type, elements, options, variable, value);
        }
    ...
    Over.prototype = {
        ...
        show: function () {
            ...
            if (this.options.url) {
                ...
                alto.ajax(this.options.url, this.options.params, function (result) {
                    if (!result) {
                        alto.msg.error(null, 'System error #1001');
                    } else if (result.bStateError) {
                        self.hide();
                        alto.msg.error('Error', result.sMsg);
                    } else {
                        if (self.$target) {
     ...
     * Plugin defenition
     */
    $.fn.over = function (options, variable, value) {
        Over.initPlugin('over', this, options);
    };

    $.fn.over.Constructor = Over;
