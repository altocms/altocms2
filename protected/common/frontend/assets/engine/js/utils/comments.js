...
ls.comments = (function ($) {
    ...
    this.defaults = {
        type: {
            topic: {
                urlAdd:      alto.routerUrl('blog') + 'ajaxaddcomment/',
                urlResponse: alto.routerUrl('blog') + 'ajaxresponsecomment/',
                urlGet:      alto.routerUrl('blog') + 'ajaxgetcomment/',
                urlUpdate:   alto.routerUrl('blog') + 'ajaxupdatecomment/'
            },
            talk: {
                urlAdd:      alto.routerUrl('talk') + 'ajaxaddcomment/',
                urlResponse: alto.routerUrl('talk') + 'ajaxresponsecomment/'
            }
        },
        classes: {
            formLoading: 'loading',
    ...
    this.init = function (options) {
        ...
        if (this.getReplyForm()) {
            ...
            // *depricated* //ls.hook.run('ls_comments_init_after', [], this);
        }
    };
    ...
    this.add = function (form, targetId, targetType) {
        ...
        alto.progressStart();
        alto.ajax(this.options.type[targetType].urlAdd, form.serializeJSON(), function (result) {
            alto.progressDone();
            ...
            if (!result) {
                ...
                // *depricated* //ls.hook.run('ls_comments_add_after', [form, targetId, targetType, result]);
            }
        }.bind(this));
    };
    ...
    this.loadComment = function(idComment, targetType, targetId, options) {
        ...
        alto.progressStart();
        this.formCommentWait();
        alto.ajax(this.options.type[targetType].urlGet, params, function(result){
            alto.progressDone();
            $that.enableFormComment();
            if (!result) {
                alto.msg.error('Error','Please try again later');
            } else if (result.bStateError) {
                alto.msg.error(null,result.sMsg);
            } else {
    ...
    this.editSubmit = function(form, targetId, targetType) {
        ...
        this.formCommentWait();
        alto.progressStart();
        alto.ajax(this.options.type[targetType].urlUpdate, form.serializeJSON(), function(result){
            alto.progressDone();
            this.enableFormComment();
            if (!result) {
                alto.msg.error('Error','Please try again later');
            ...
            if (result.bStateError) {
                ...
                if (result.sMsg) alto.msg.notice(null,result.sMsg);
                this.formCommentText('');
    ...
    this.load = function (idTarget, typeTarget, selfIdComment, bNotFlushNew) {
        ...
        alto.progressStart();
        alto.ajax(this.options.type[typeTarget].urlResponse, params, function (result) {
            alto.progressDone();
            ...
            if (!result) {
                ...
                this.checkFolding();
                this.checkEditTimers();
                // *depricated* //ls.hook.run('ls_comments_load_after', [idTarget, typeTarget, selfIdComment, bNotFlushNew, result]);
            }
        }.bind(this));
    };
    ...
    this.inject = function (idCommentParent, idComment, sHtml) {
        ...
        // *depricated* //ls.hook.run('ls_comment_inject_after', arguments, newComment);
    };


    // Удалить/восстановить комментарий
    this.toggle = function (obj, commentId) {
        var url = alto.routerUrl('ajax') + 'comment/delete/';
        ...
        alto.ajax(url, params, function (result) {
            ...
            if (!result) {
                ...
                // *depricated* //ls.hook.run('ls_comments_toggle_after', [obj, commentId, result]);
            }
        }.bind(this));
    };
