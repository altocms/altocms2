...
(function ($, ls) {
    ...
    altoMultiUploader.prototype = {
                    ...
                    if (data.other.length) {
                        ...
                        if (errors) {
                            if (errors.maxSize !== undefined) {
                                alto.msg.error(null, $this.options.maxSizeError);
                            } else if (errors.maxWidth !== undefined) {
                                alto.msg.error(null, $this.options.maxWidthError);
                            } else if (errors.maxHeight !== undefined) {
                                alto.msg.error(null, $this.options.maxHeightError);
                            }
                        }
                    }
                };
            ...
            $($this.options.form).fileapi({
                ...
                onFileComplete: function (evt, uiEvt) {
                    if (uiEvt && uiEvt.status && uiEvt.status >= 400) {
                        alto.msg.error('HTTP Error ' + uiEvt.status, uiEvt.statusText ? uiEvt.statusText : '');
                    } else {
                        /*jslint nomen: true*/
                ...
                .sortable({
                    stop: function () {
                        ...
                        if (elements.length > 0) {
                            ...
                            /*jslint unparam: false*/
                            alto.ajax($this.options.url.sort,
                                {
                                    target: $this.options.target,
                                    target_id: $this.options.targetId,
                                    order: order
        ...
        addImg: function (data) {
            ...
            alto.log('add img', data);
            if (!data) {
                alto.msg.error(null, 'System error #1001');
            } else if (typeof data === 'string') {
                alto.msg.error(null, 'System error #1003');
            } else if (data.bStateError) {
                alto.msg.error(data.sMsgTitle, data.sMsg);
            } else {
        ...
        setCover: function (id) {
            ...
            // *depricated* //ls.hook.run('uploader_cover_start', [$this.options]);
            alto.ajax($this.options.url.cover,
                {
                    target: $this.options.target,
                    target_id: $this.options.targetId,
                    resource_id: id
                ...
                function (result) {
                    alto.progressDone();
                    $this.blockButtons = false;
                    // *depricated* //ls.hook.run('uploader_cover_stop', [$this.options]);
                    if (!result) {
                        alto.msg.error(null, 'System error #1001');
                        ...
                        // *depricated* //ls.hook.run('uploader_set_cover_after', [$this.options, result]);
                    }
                });
        ...
        remove: function (id) {
            ...
            alto.progressStart();
            $this.blockButtons = true;
            // *depricated* //ls.hook.run('uploader_progress_start', [$this.options]);
            alto.ajax($this.options.url.remove,
                {
                    target: $this.options.target,
                    target_id: $this.options.targetId,
                    resource_id: id
                ...
                function (result) {
                    alto.progressDone();
                    $this.blockButtons = false;
                    // *depricated* //ls.hook.run('uploader_progress_stop', [$this.options]);
                    if (!result) {
                        alto.msg.error(null, 'System error #1001');
                    } else if (result.bStateError) {
                        alto.msg.error(null, result.sMsg);
                        ...
                        alto.msg.notice(null, result.sMsg);
                        // *depricated* //ls.hook.run('uploader_remove_image_after', [$this.options, result]);
                    }
                });
        ...
        processDescriptionQueue: function (callback) {
            ...
            alto.progressStart();
            $this.blockButtons = true;
            // *depricated* //ls.hook.run('uploader_description_start', [$this.options]);
            alto.ajax($this.options.url.description, data,
                /**
                 * Обработчик результата, пришедшего от сервера
                 * @param {{bStateError: {boolean}, sMsg: {string}}} result
                 */
                function (result) {
                    alto.progressDone();
                    // *depricated* //ls.hook.run('uploader_description_stop', [$this.options]);
                    $this.blockButtons = false;
                    ...
                    if (!result) {
                        ...
                        // *depricated* //ls.hook.run('uploader_add_description_after', [$this.options, result]);
                    }
                });
        },
    ...
    $.fn.altoMultiUploader.defaultOptions = {
        ...
        url: {
            upload: alto.routerUrl('uploader') + 'multi-image/',
            remove: alto.routerUrl('uploader') + 'remove-image-by-id/',
            description: alto.routerUrl('uploader') + 'description/',
            cover: alto.routerUrl('uploader') + 'cover/',
            sort: alto.routerUrl('uploader') + 'sort/'
        },
        previewCrop: '400fit',
        tmp: true,
        submitForm: ''
