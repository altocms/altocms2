...
ls.poll = (function ($) {
    ...
    var defaults = {
        // Роутер голосования
        voteUrl: alto.routerUrl('ajax') + 'vote/poll/',
    ...
    this.addItem = function (poll) {
        ...
        if (items.length > 1) {
            if (items.length >= this.options.iMaxItems) {
                alto.msg.error(null, alto.lang.get('topic_question_create_answers_error_max'));
                return false;
            }
    ...
    this.vote = function (button, value) {
        ...
        alto.progressStart();
        alto.ajax(this.options.voteUrl, params, function (result) {
            alto.progressDone();
            if (!result) {
                ...
                // *depricated* //ls.hook.run('ls_pool_vote_after', [pollId, value, result], poll);
            }
        });
        return false;
    };
