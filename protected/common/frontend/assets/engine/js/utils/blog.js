...
ls.blog = (function ($) {
    ...
    this.toggleJoin = function (button, idBlog) {
        var url = alto.routerUrl('blog') + 'ajaxblogjoin/',
        ...
        alto.ajax(url, params, function (result) {
            ...
            if (!result) {
                ...
                var text = result.bState ? alto.lang.get('blog_leave') : alto.lang.get('blog_join');
    ...
    this.addInvite = function (idBlog) {
        var addUserList = $(this.options.selectors.addUserList),
            ...
            url = alto.routerUrl('blog') + 'ajaxaddbloginvite/';
        ...
        alto.ajax(url, params, function (result) {
            ...
            if (!result) {
                ...
                $.each(result.aUsers, function (index, item) {
                    if (item.bStateError) {
                        alto.msg.error(item.sMsgTitle ? item.sMsgTitle : 'Error', item.sMsg);
                    } else {
                        if (item.sMsg) {
                            alto.msg.notice(item.sMsgTitle ? item.sMsgTitle : null, item.sMsg);
                        }
    ...
    this.repeatInvite = function (idUser, idBlog) {
        var url = alto.routerUrl('blog') + 'ajaxrebloginvite/',
    ...
    this.removeInvite = function (idUser, idBlog) {
        var url = alto.routerUrl('blog') + 'ajaxremovebloginvite/',
            params = {idUser: idUser, idBlog: idBlog};
    ...
    this.loadInfo = function (idBlog) {
        var url = alto.routerUrl('blog') + 'ajaxbloginfo/',
            params = {idBlog: idBlog},
    ...
    this.loadInfoType = function (type) {
        $('#blog_type_note').text(alto.lang.get('blog_create_type_' + type + '_notice'));
    };
    ...
    this.searchBlogs = function (form) {
        form = $(form);
        var url = alto.routerUrl('blogs') + 'ajax-search/';
    ...
    this.toggleInfo = function () {
        ...
        if (more.hasClass('expanded')) {
            more.html(alto.lang.get('blog_fold_info'));
        } else {
            more.html(alto.lang.get('blog_expand_info'));
        }
    ...
    this.loadBlogsByCategory = function (id) {
        var url = alto.routerUrl('ajax') + 'blogs/get-by-category/',
            params = {id: id},
        ...
        if (id !== '0') {
            ...
            $blogs.append('<option>' + alto.lang.get('blog') + '</option>');
        }
    };
