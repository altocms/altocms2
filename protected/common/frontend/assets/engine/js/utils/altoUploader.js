...
(function ($, ls) {
    ...
    altoUploader.prototype = {
        ...
        uploadDirect: function () {
            ...
            $this.blockButtons = true;
            // *depricated* //ls.hook.run('uploader_progress_start', [$this.options]);
            alto.ajax($this.options.url.direct, {
                ...
                function (result) {
                    ...
                    if (!result) {
                        ...
                        // *depricated* //ls.hook.run('uploader_upload_image_after', [$this.options, result]);
                    }
                });
        ...
        uploadImageModalCrop: function (sImgFile) {
            ...
            if ($this.options.title !== undefined) {
                $this.$modal.find('.modal-title').text(alto.lang.get($this.options.title) || $this.options.title);
            }
            if ($this.options.help !== undefined) {
                $this.$modal.find('.js-crop_img-help').text(alto.lang.get($this.options.help) || $this.options.help);
            }
        ...
        uploadImageCropSubmit: function (button) {
            ...
            $this.blockButtons = true;
            // *depricated* //ls.hook.run('uploader_progress_start', [$this.options]);
            alto.ajax($this.options.url.crop, params,
                /**
                 * Обработчик результата, пришедшего от сервера
                 * @param {{bStateError: {boolean}, sMsg: {string}, sFilePreview: {string}, sFile: {string}}} result
                 */
        ...
        uploadImageCropCancel: function (button) {
            ...
            $this.blockButtons = true;
            // *depricated* //ls.hook.run('uploader_progress_start', [$this.options]);
            alto.ajax($this.options.url.cancel, {
                    target: $this.options.target,
                    target_id: $this.options.targetId
                },
                /**
                ...
                function (result) {
                    ...
                    if (!result) {
                        ...
                        $this.$modal.modal('hide');
                        // *depricated* //ls.hook.run('ls_uploader_cancel_after', [result]);
                    }
    ...
    $.fn.altoUploader.defaultOptions = {
        ...
        url: {
            upload: alto.routerUrl('uploader') + 'upload-image/',
            remove: alto.routerUrl('uploader') + 'remove-image/',
            cancel: alto.routerUrl('uploader') + 'cancel-image/',
            direct: alto.routerUrl('uploader') + 'direct-image/',
            crop: alto.routerUrl('uploader') + 'resize-image/'
        },
        previewCrop: '400fit'
    };
