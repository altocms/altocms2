...
ls.photoset = ( function ($) {
    ...
    this.initSwfUpload = function (opt) {
        ...
        $(ls.swfuploader).unbind('load').bind('load', function () {
            ...
            $(this.swfu).bind('eReady', function(e){
                alto.log('[ready]', $(e.target.movieElement));
    ...
    this.swfHandlerFileQueueError = function (e, file, errorCode, message) {
        alto.msg.error('Error: ' + errorCode, message);
    ...
    this.deletePhoto = function (id) {
        var photo = $(this._itemId(id)),
            ...
            html = alto.lang.get('topic_photoset_photo_delete_confirm');
        ...
        ls.modal.confirm(html, {
            onConfirm: function() {
                ...
                alto.progressStart();
                ls.ajaxPost(alto.routerUrl('content') + 'photo/delete', {'id': id}, function (response) {
                    alto.progressDone();
                    if (!response) {
                        alto.msg.error(null, 'System error #1001');
                    } else if (response.bStateError) {
    ...
    this.setPreview = function (id) {
        ...
        $('.marked-as-preview').each(function (index, el) {
            ...
            $('#photo_preview_state_' + tmpId).html('<a href="javascript:ls.photoset.setPreview(' + tmpId + ')" class="mark-as-preview link-dotted">' + alto.lang.get('topic_photoset_mark_as_preview') + '</a>');
        });
        $(ls.photoset._itemId(id)).addClass('marked-as-preview');
        $('#photo_preview_state_' + id).html(alto.lang.get('topic_photoset_is_preview'));
    };
    ...
    this.setPreviewDescription = function (id) {
        ...
        alto.progressStart();
        ls.ajaxPost(alto.routerUrl('content') + 'photo/description', {'id': id, 'text': text}, function (response) {
                alto.progressDone();
                if (!response) {
                    alto.msg.error(null, 'System error #1001');
                } else if (response.bStateError) {
                    alto.msg.error('Error', response.sMsg ? response.sMsg : 'Please try again later');
                } else if (response.sMsg) {
                    alto.msg.notice('', response.sMsg);
                }
            }
        )
    };
    ...
    this.getMore = function (topic_id) {
        ...
        alto.progressStart();
        ls.ajaxGet(alto.routerUrl('content') + 'photo/getmore', params, function (response) {
            this.isLoading = false;
            alto.progressDone();
            if (!response) {
                alto.msg.error(null, 'System error #1001');
            } else if (response.bStateError) {
                alto.msg.error(response.sMsgTitle ? response.sMsgTitle : 'Error', response.sMsg ? response.sMsg : 'Please try again later');
            } else {
                if (response.photos) {
    ...
    this.upload = function () {
        ...
        ls.ajaxSubmit(alto.routerUrl('content') + 'photo/upload/', form, function (response) {
            if (!response) {
                alto.msg.error(null, 'System error #1001');
            } else if (response.bStateError) {
                alto.msg.error(response.sMsgTitle, response.sMsg);
