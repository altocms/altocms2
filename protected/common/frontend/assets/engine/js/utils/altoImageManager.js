...
(function ($, ls) {
                    ...
                    if (!result || result.bStateError == true) {
                        if ($.type(error) == 'function') {
                            ...
                            if (result && result.sMsg) {
                                alto.msg.error(result.sMsgTitle ? result.sMsgTitle : '', result.sMsg);
                            } else {
                                alto.msg.error(null, 'System error #1001');
                            }
                        }
    ...
    $.fn.altoImageManager.defaultOptions = {
        url: {
            loadTree: alto.routerUrl('ajax') + 'image-manager-load-tree/',
            loadImages: alto.routerUrl('ajax') + 'image-manager-load-images/'
        }
    };
