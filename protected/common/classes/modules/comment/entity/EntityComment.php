...
class ModuleComment_EntityComment extends Entity
{
    ...
    public function getTargetParentId() {
        ...
        return (int)$this->getProp('target_parent_id');
    }
    ...
    public function isBad() {
        ...
        if ($this->getRating() <= \C::get('module.comment.bad')) {
            return true;
        }
        return false;
    }
    ...
    public function isEditable($bByAuthor = null) {
        ...
        if ($this->getTargetType() != 'talk' && ($oUser = \E::User())) {
            if ($bByAuthor === false || is_null($bByAuthor)) {
                ...
                if (($oBlog = $this->getTargetBlog()) && \E::Module('ACL')->CheckBlogEditComment($oBlog, $oUser)) {
                    return true;
                }
            }
            if ($bByAuthor === true || is_null($bByAuthor)) {
                // author of comment can edit comment in time limit
                if (($oUser->GetId() == $this->getUserId()) && \C::get('module.comment.edit.enable') && !$this->getDelete()) {
    ...
    public function isDeletable() {
        ...
        if ($this->getTargetType() !== 'talk' && ($oUser = \E::User())) {
            ...
            if (($oBlog = $this->getTargetBlog()) && \E::Module('ACL')->CheckBlogDeleteComment($oBlog, $oUser)) {
                return true;
            }
        }
        return false;
    ...
    public function isVoteExpired() {
        ...
        $nTimeLimit = \C::get('acl.vote.comment.limit_time');
        if (!$nTimeLimit) {
            return false;
        }
    ...
    public function getLink() {
        ...
        if ($oTarget = $this->getTarget()) {
            return $oTarget->getLink() . '#comment' . $this->getId();
        }
        return null;
    }
