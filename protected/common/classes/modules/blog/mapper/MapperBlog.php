...
class ModuleBlog_MapperBlog extends Mapper {
    ...
    protected $aBlogOrderAllow = ['blog_id', 'blog_title', 'blog_rating', 'blog_count_user', 'blog_count_topic'];
    ...
    public function getBlogUsers($aFilter, &$iCount = null, $iCurrPage = null, $iPerPage = null)
    {
        ...
        if ((func_num_args() == 1) || is_null($iPerPage)) {
            ...
            $sql .= " LIMIT " . (($iCurrPage - 1) * $iPerPage) . ", " . (int)$iPerPage;
    ...
    public function getBlogsIdByUrl($xUrl) {
        ...
        if ($bSingle) {
            ...
            return $aResult['data'] ? $aResult['data'] : [];
        }
    }
    ...
    public function getBlogsIdByOwnerId($iUserId) {
        ...
        $aFilter = \E::Module('Blog')->getNamedFilter('default');
        $aFilter['user_id'] = (int)$iUserId;
        $aCriteria = array(
            'filter' => $aFilter,
        );
        $aResult = $this->GetBlogsIdByCriteria($aCriteria);
    ...
    public function getBlogsIdByFilter($aFilter) {
        ...
        if (isset($aFilter['user_id'])) {
            $aFilter['user_id'] = (int)$aFilter['user_id'];
            if (!$aFilter['user_id']) {
                return [];
            }
        }
    ...
    public function getCloseBlogsId($oUser = null) {
        ...
        $aTypes = \E::Module('Blog')->getCloseBlogTypes($oUser);
            ...
            foreach ($aResult as $oBlogType) {
                ...
                $oBlogType->setBlogsCount(
                    isset($aStat[$oBlogType->getTypeCode()])
                        ? (int)$aStat[$oBlogType->getTypeCode()]
                        : 0
                );
            }
    ...
    public function getBlogsIdByCriteria($aCriteria = array()) {
        ...
        if (isset($aFilter['not_blog_id'])) {
            if (!is_array($aFilter['not_blog_id'])) {
                $aFilter['not_blog_id'] = [(int)$aFilter['not_blog_id']];
            }
        }
        ...
        if (isset($aCriteria['order'])) {
            ...
            if (!is_array($aCriteria['order'])) {
                $aCriteria['order'] = [$aCriteria['order']];
            }
        ...
        if ($nCalcTotal) {
            if (($nOffset == 0) && ($nLimit > 0) && (count($aResult['data']) < $nLimit)) {
                $aResult['total'] = count($aResult['data']);
            } else {
                $sLastQuery = trim(\E::Module('Database')->getLastQuery());
