...
class ModuleTopic_EntityTopic extends Entity {
    ...
    public function init() {
        ...
        $this->aValidateRules[] = array(
            ...
            'allowEmpty' => false,
            'label' => \E::Module('Lang')->get('topic_create_title'),
            'on' => array('topic'),
        );
        $this->aValidateRules[] = array(
            'question_title', 'string', 'max' => 200, 'min' => 2,
            'allowEmpty' => true,
            'label' => \E::Module('Lang')->get('topic_create_question_title'),
            'on' => array('topic'),
        );
        $this->aValidateRules[] = array(
            'topic_text_source', 'string', 'max' => \C::get('module.topic.max_length'), 'min' => 2,
            'allowEmpty'                         => false,
            'label'                              => \E::Module('Lang')->get('topic_create_text'),
            'on'                                 => array('topic'),
        );
        $this->aValidateRules[] = array(
            'topic_tags', 'tags', 'count' => 15,
            'allowEmpty' => \C::get('module.topic.allow_empty_tags'),
            'label' => \E::Module('Lang')->get('topic_create_tags'),
            'on' => array('topic'),
        );
        $this->aValidateRules[] = array(
            'blog_id', 'blog_id',
        ...
        $this->aValidateRules[] = array(
            'link_url', 'url',
            'allowEmpty' => true,
            'label' => \E::Module('Lang')->get('topic_link_create_url'),
            'on' => array('topic'),
        );
    }
    ...
    public function ValidateTopicType($sValue, $aParams) {
        ...
        if (\E::Module('Topic')->IsAllowTopicType($sValue)) {
            return true;
        }
        return \E::Module('Lang')->get('topic_create_type_error');
    }
    ...
    public function ValidateTopicUnique($sValue, $aParams) {
        ...
        if ($oTopicEquivalent = \E::Module('Topic')->getTopicUnique($this->getUserId(), $this->getTextHash())) {
            if ($iId = $this->getId() and $oTopicEquivalent->getId() == $iId) {
                return true;
            }
            return \E::Module('Lang')->get('topic_create_text_error_unique');
        }
        return true;
    }
    ...
    public function ValidateBlogId($sValue, $aParams) {
        ...
        if (\E::Module('Blog')->getBlogById((string)$sValue)) {
            return true;
        }
        return \E::Module('Lang')->get('topic_create_blog_error_unknown');
    }
    ...
    public function getContentType() {
        ...
        if (is_null($oContentType)) {
            $oContentType = \E::Module('Topic')->getContentType($this->getType());
    ...
    public function getUser() {
        ...
        if (!$this->getProp('user')) {
            $this->setProp('user',   \E::Module('User')->getUserById($this->getUserId()));
        }
    ...
    public function getBlog() {
        ...
        if (!$oBlog) {
            ...
            if ($iBlogId) {
                $oBlog = \E::Module('Blog')->getBlogById($iBlogId);
    ...
    public function getIsAllowDelete() {
        ...
        if ($oUser = \E::User()) {
            return \E::Module('ACL')->isAllowDeleteTopic($this, $oUser);
        }
        return false;
    }
    ...
    public function getIsAllowEdit() {
        ...
        if ($oUser = \E::User()) {
            return \E::Module('ACL')->isAllowEditTopic($this, $oUser);
        }
        return false;
    }
    ...
    public function getSubscribeNewComment() {
        ...
        return \E::Module('Subscribe')->getSubscribeByTargetAndMail(
    ...
    public function getTrackNewComment() {
        ...
        return \E::Module('Subscribe')->getTrackByTargetAndUser('topic_new_comment', $this->getId(), $oUserCurrent->getId());
    }
    ...
    public function buildMresourcesList() {
        ...
        if ($aTextLinks) {
            foreach($aTextLinks as $aLink) {
                /** @var ModuleMedia_EntityMediaRel $oMresource */
                $oMresource = \E::getEntity('Media_MediaRel');
                $oMresource->setUrl(\E::Module('Media')->normalizeUrl($aLink['link']));
                $oMresource->setType($aLink['type']);
                $oMresource->setUserId($this->getUserId());
                $aResources[] = $oMresource;
                // if image is derived from another image then add original media
                if ($oMresource->isDerivedImage()) {
                    // get original path and make one more mresorce
                    $sOriginal = $oMresource->GetOriginalPathUrl();

                    $oMresource = \E::getEntity('Media_MediaRel');
                    $oMresource->setUrl(\E::Module('Media')->normalizeUrl($sOriginal));
                    $oMresource->setType($aLink['type']);
                    $oMresource->setUserId($this->getUserId());
                    $aResources[] = $oMresource;
                }
        ...
        return \E::Module('Media')->BuildMresourceHashList($aResources);
    }
    ...
    public function getPhotosetPhotos($iFromId = null, $iCount = null) {
        ...
        $aPhotos = \E::Module('Media')->getMresourcesRelByTarget('photoset', $this->getId());
        $aResult = [];
     ...
     *
     * @return ModuleMedia_EntityMediaRel|null
     */
    public function getPhotosetMainPhoto() {
        ...
        if ($oPhotosetCover === null) {
            ...
            foreach($aImages as $oImage) {
                if ($oImage->getType() & ModuleMedia::TYPE_PHOTO_PRIMARY) {
    ...
    public function getPhotosetMainPhotoLink($xSize = null) {
        ...
        if ($oImage) {
            if ($xSize) {
                return \E::Module('Uploader')->ResizeTargetImage($oImage->getImageUrl(), $xSize);
            }
    ...
    {
        return (int)$this->getTopicIndexIgnore() == self::INDEX_IGNORE_LOCK;
    }
    ...
    public function getSingleImage($nId, $sSize) {
        ...
        if ($this->getField($nId)) {
            ...
            return \E::Module('Uploader')->getTargetImageUrl(
    ...
    public function setBlogId($data) {
        ...
        $this->setProp('blog_id', is_numeric($data) ? (int)$data : null);
    }
    ...
    public function getImages() {
        ...
        if ($aMediaTypes) {
            foreach($aMediaTypes as $sType => $aMedia) {
                /** @var ModuleMedia_EntityMediaRel $oMedia */
