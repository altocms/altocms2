...
class ModuleTopic_MapperTopic extends Mapper 
{
     ...
     * @return int|bool
     */
    public function addTopicContent($oTopic) {
    ...
    public function getTopicUnique($nUserId, $sHash) {
        ...
        $sql = "SELECT topic_id FROM ?_topic
			...
			LIMIT 0,1
				";
        return (int)$this->oDb->selectCell($sql, $sHash, $nUserId ? $nUserId : DBSIMPLE_SKIP);
    }
     ...
     * @return array
     */
    public function getTopicsRatingByDate($sDate, $iLimit, $aExcludeBlog = [])
    {
        $sql
            = "SELECT
						t.topic_id
    ...
    public function recalcCountOfComments($iTopicId)
    {
        $sql = "UPDATE ?_topic
			...
			WHERE
				topic_id = ?d
		";
        return false !== $this->oDb->query($sql, $iTopicId, $iTopicId);
    }
    ...
    protected function buildFilter($aFilter)
    {
        ...
        if (isset($aFilter['blog_id'])) {
            ...
            $sWhere .= " AND t.blog_id IN ('" . implode("','", $aFilter['blog_id']) . "')";
        }
        if (isset($aFilter['topic_id']) && is_array($aFilter['topic_id'])) {
            $sWhere .= " AND t.topic_id IN ('" . implode("','", $aFilter['topic_id']) . "')";
        }
        if (isset($aFilter['blog_type']) && is_array($aFilter['blog_type'])) {
            $aBlogTypes = [];
            $aOrClauses = [];
            $aFilter['blog_type'] = F::Array_FlipIntKeys($aFilter['blog_type'], 0);
            foreach ((array)$aFilter['blog_type'] as $sType => $aBlogsId) {
                if ($aBlogsId) {
                    // 'type'=>array('id1', 'id2') - blog type & blogs id
                    if ($sType === '*') {
                        $aOrClauses[] = "(t.blog_id IN ('" . implode("','", $aBlogsId) . "'))";
                    } else {
                        $aOrClauses[] = "b.blog_type='" . $sType . "' AND t.blog_id IN ('" . implode("','", $aBlogsId) . "')";
                    }
                } else {
                    // blog type only
                    $aBlogTypes[] = "'" . $sType . "'";
            ...
            if ($aBlogTypes) {
                $aOrClauses[] = '(b.blog_type IN (' . implode(',', $aBlogTypes) . '))';
            }
            if ($aOrClauses) {
    ...
    public function moveTopicsByArrayId($aTopics, $nBlogIdNew)
    {
        ...
        return $this->moveTopicsByFilter($nBlogIdNew, ['topic_id' => $aTopics]);
    }
    ...
    {
        return $this->moveTopicsByFilter($nBlogIdNew, ['blog_id' => $nBlogIdOld]);
    }
    ...
    public function moveTopicsByFilter($nBlogIdNew, $aFilter)
    {
        ...
        $oBlogType = \E::Module('Blog')->getBlogTypeById($nBlogIdNew);
        if ($oBlogType) {
    ...
    public function deleteContentType($aContentTypesId)
    {
        if (!is_array($aContentTypesId)) {
            $aContentTypesId = [(int)$aContentTypesId];
        }
        $sql = "
