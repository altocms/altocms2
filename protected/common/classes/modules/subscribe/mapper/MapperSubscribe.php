...
class ModuleSubscribe_MapperSubscribe extends Mapper {
    ...
    {
        $aOrderAllow = ['id', 'date_add', 'status'];
        $sOrder = '';
        ...
        if (isset($aFilter['exclude_mail']) && !is_array($aFilter['exclude_mail'])) {
            $aFilter['exclude_mail'] = [$aFilter['exclude_mail']];
        }
    ...
    public function getTracks($aFilter, $aOrder, &$iCount, $iCurrPage, $iPerPage)
    {
        ...
        $aRows = $this->oDb->selectPage(
            ...
            (isset($aFilter['exclude_user']) && count($aFilter['exclude_user'])) ? $aFilter['exclude_user'] : DBSIMPLE_SKIP,
            isset($aFilter['key']) ? $aFilter['key'] : DBSIMPLE_SKIP,
            isset($aFilter['status']) ? $aFilter['status'] : DBSIMPLE_SKIP,
            (isset($aFilter['only_new']) && $aFilter['only_new']) ? 1 : DBSIMPLE_SKIP,
            ($iCurrPage - 1) * $iPerPage, $iPerPage
        );
        if ($aRows) {
